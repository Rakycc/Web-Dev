...
class ManageInfo extends Model
{
    ...
    public function checkLogin($username, $password)
    {
        ...
        if ($admin['password'] == md5($password)) {
         ...
         $sql="SELECT * FROM log_user WHERE datediff(week,[dateadd],getdate())=0 ";

        $res=Db::name('log_user')->whereTime('operate_time','>','this week')->where("user_id","=","{$admin['id']}")->where("operate_type","=","1")->select();
        Session::set("dl_num",count($res));
