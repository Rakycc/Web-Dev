...
class Index extends Controller
{
    ...
    public function userLogin($name, $number) {
        $res = Db::table("user_info")->where('work_id ='.$number)->where('is_delete=0')->select(); 
        if ($res) {
            $userData = [
                "name"=>$name,
    ...
    public function checkUserLogin($number) {
        return Db::table('user_info')->where('work_id='.$number)->where('is_delete=0')->where('have_login=1')->select();
    }
    ...
    public function addUser($name,$number){
        ...
        if($res){
            $userData = [
                ...
                "depart_id"=>$res[0]['depart_id'],
                "position_id"=>$res[0]['position_id'],
                "have_login"=>$res[0]['have_login'],
                "is_delete"=>0,
            ];
            Db::table('user_info')->where('work_id='.$number)->update($userData);
        } else {
            Db::table('user_info')
            ->data(['name'=> $name,'work_id'=>$number,'type_id'=>0,'depart_id'=>105,'position_id'=>50,'is_delete'=>0])->insert();
        }
    }
    ...
    //  return $this->fetch();
        ...
        if($accessToken){
            ...
            if(!$res){
                ...
                if(!$this->checkUserLogin($userInfo['card_number'])) {
            ...
            if ($userName && $number) {
                $this->userLogin($userName,$number);
    ...
    public function checkDate()
    {
        ...
        $info = Db::table('schedule_info')
            ...
            ->field('d.name as time, e.name as place, f.name as item, b.id as userid,a.date as week,a.note as note')
            ->select();
        for($i = 0;$i<count($info);$i++){
            $info[$i]['week'] = $this->get_week($info[$i]['week']);
        }
        ...
        $info2 = Db::table('schedule_default')
            ...
            ->join('schedule_place','a.place_id = e.id')
            ->join('schedule_item','a.item_id = f.id')
            ->field('d.name as time, e.name as place, f.name as item, b.id as userid,a.day as week,a.note as note')
            ->select();
...
/*        if (curl_errno($ch)) {
    ...
    public function getView(){
        ...
        $this->redirect('wx_calendar/index', ['userid' => $userid, 'date' =>$date, 'view'=>'']);
    }
}